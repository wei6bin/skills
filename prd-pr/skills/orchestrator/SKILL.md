...
3. **Refactor / simplify the whole story.** Dispatch `agent_type: "prd-pr:impl-simplify"` once, passing the user-story folder path, the branch-point ref, and the full changed-file list. The agent is a thin **context boundary**: it invokes Claude Code's built-in `simplify` skill with `{branch-point}..HEAD` as its target, re-runs the test suite to prove behaviour is unchanged, and commits. The skill fans its review out to 4 cleanup agents in parallel (reuse, simplification, efficiency, altitude) and applies the deduped fixes — that fan-out and the applying both land in the agent's context, not yours. Dispatch it rather than invoking `/simplify` here — a whole-story refactor reads every changed file and every diff hunk, and spending that in your session would evict the plan docs and Return Reports you still need for Phases 9 and 10. `simplify` is quality-only and does not hunt for bugs, which is why it runs *after* step 2's reviewers signed off. Read its Return Report: the suite must be green (re-run it yourself if the report is missing that line), and anything under "Reported, not fixed" that is a blocker goes back to the owning implementer before you continue. Mark the "Refactor / simplify" row ✅.
