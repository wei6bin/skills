...
3. **Refactor / simplify the whole story.** Invoke Claude Code's built-in `simplify` skill (main session, not a subagent) - `/simplify`. It reviews the changed code for reuse, simplification, efficiency, and altitude cleanups and applies the fixes; it is quality-only and deliberately does not hunt for bugs, which is why it runs *after* step 2's reviewers have already signed off on correctness. Scope it to the story's diff by naming the branch point → `HEAD` range in the invocation; on a dedicated story branch its default (the branch's changed code) is already that diff. Once it finishes, re-run the affected test suites yourself: simplification must not change behaviour, and any failure it introduced is fixed before the row is marked. Mark the "Refactor / simplify" row ✅.
