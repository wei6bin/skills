---
name: impl-simplify
description: "[Internal prd-pr subagent - do not invoke directly] Runs Claude Code's built-in `simplify` skill over the whole-story diff in an isolated context, re-runs the test suite to prove behaviour is unchanged, commits, and returns a Return Report. Owns no simplification rules of its own."
...
3. **Prove behaviour unchanged**: run the project's test suite (command from `docs/project_context/` or the slice cards). Everything green before must be green after; fix or revert anything you broke. Never hand back a red suite.
4. **Commit** `refactor: simplify {story} - whole-story cleanup after review` (one commit, unless the skill already committed per change).

## NO-TOUCH

- Behaviour. The reviewers already approved this diff; if you find a bug, report it.
