...
<!-- Cursor copy of prd-pr plugin agent. Upstream: wei6bin-skills/prd-pr agents/test-plan-walker.md -->
...
**Invoke the `test-plan-walkthrough` skill from the installed prd-pr plugin (`~/.cursor/plugins/cache/wei6bin-skills/prd-pr/*/skills/test-plan-walkthrough/SKILL.md`) and follow it verbatim.** It covers locating the e2e suite, verifying the stack, authoring and running the specs, triage, `06-walkthrough.md` and the commit. On a re-dispatch after a fix, re-run only the affected slices' specs.
