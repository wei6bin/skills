...
Derive the mode from the slice card's **story-point size** (or honour a `reuse: lite|full` token if the orchestrator put one in your scope), and pass it to the `backend-implementer` skill. It tunes how hard the reuse ladder is enforced — it never changes what the ACs require:
