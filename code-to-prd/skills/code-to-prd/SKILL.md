---
name: code-to-prd
description: "Reverse-engineer any codebase into a complete Product Requirements Document (PRD). Analyzes routes, components, state management, API integrations, and user interactions to produce business-readable documentation detailed enough for engineers or AI agents to fully reconstruct every page and endpoint. Works with frontend frameworks (React, Vue, Angular, Svelte, Next.js, Nuxt), backend frameworks (NestJS, Django, Express, FastAPI), .NET / ASP.NET (Web API, WCF, Web Forms), native Android (Java/Kotlin), native iOS (Swift/Objective-C), and fullstack applications. Use when users mention: generate PRD, reverse-engineer requirements, code to documentation, extract product specs from code, document page logic, analyze page fields and interactions, create a functional inventory, write requirements from an existing codebase, document API endpoints, analyze backend routes, or reverse-engineer a .NET/Android/iOS app."
license: MIT
metadata:
  updated: 2026-07-16
  tier: STANDARD
...
.NET / ASP.NET directories:
- Solution/project files (*.sln, *.csproj) — identify the stack before scanning
- Controllers (*Controller.cs) — Web API/MVC route handlers ([ApiController]/[Route]/[Http*])
- Service contracts (I*.cs with [ServiceContract]) — WCF operations ([OperationContract]/[WebInvoke]/[WebGet])
- Web Forms pages (*.aspx + *.aspx.cs code-behind) — server-rendered pages
- Models/Domain/Entities/DTO/DO/VO — POCOs, EF DbContext + DbSet<T>

Native Android directories:
- build.gradle(.kts), AndroidManifest.xml — identify the stack, list declared activities/fragments
- Activities/Fragments (*.java, *.kt extending Activity/AppCompatActivity/Fragment) — screens
- API layer — Retrofit interfaces (@GET/@POST/...), or raw HttpURLConnection/OkHttp/Volley calls
- Models (model/, pojo/, entity/ dirs, @Entity, @SerializedName, Kotlin data class)
- res/layout/*.xml, res/values/strings.xml — control ids and display strings

Native iOS directories:
- *.xcodeproj/Podfile, *.swift/*.m source — identify the stack
- View controllers (UIViewController subclasses), storyboard/XIB scenes — screens
- API layer — URLSession/Alamofire/AFNetworking calls, URL string literals
