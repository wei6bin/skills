...
Detected via `.sln`/`.csproj` files (framework id `dotnet` for API/WCF-only backends, `aspnet` when `.aspx` pages are present).

| Aspect | Where to Look |
|--------|--------------|
| Web API / MVC routes | `[ApiController]`, `[Route("prefix")]` on the controller class, `[HttpGet/Post/Put/Delete/Patch("action")]` on each action — action route is combined with the controller prefix |
| WCF operations | `[ServiceContract]` interface, `[OperationContract]` per method, `[WebInvoke(Method="POST", UriTemplate="/path")]` / `[WebGet(UriTemplate="/path")]` for the HTTP binding |
| Web Forms pages | Each `*.aspx` is a page; `<%@ Page CodeBehind="X.aspx.cs" Inherits="Namespace.X" %>` links it to its code-behind class |
| Models | POCO classes under `Model(s)/`, `Domain/`, `Entities/`, `DTO/`, or named `*VO`/`*DO`/`*DTO`/`*Entity`/`*Model`; EF `DbContext` subclass with `DbSet<T>` properties |
| Field constraints | Data annotations: `[Key]`, `[Required]`, `[MaxLength(n)]`, `[Table("name")]` |
| Enums | `enum Name { A, B, C }` — members may carry `[EnumMember(Value = "Y")]` for their wire-format value |
| Auth | `[Authorize]` / `[AllowAnonymous]` on controllers or actions |

## Native Android (Java/Kotlin)

Detected via `build.gradle`(.kts) + `AndroidManifest.xml`.

| Aspect | Where to Look |
|--------|--------------|
| Screens | `<activity>`/`<fragment>` entries in `AndroidManifest.xml`; classes `extends Activity/AppCompatActivity/Fragment` (Java) or `: Activity()/Fragment()` (Kotlin) — screen name = class name |
| Fields/labels | `res/values/strings.xml` display strings; `res/layout/*.xml` control ids as a rough field list |
| API calls | Retrofit interfaces (`@GET/@POST/@PUT/@DELETE/@PATCH("path")`); also raw `HttpURLConnection`/OkHttp/Volley URL string literals when Retrofit isn't used |
| Models | POJOs (plain fields with getters/setters), Room `@Entity` classes, Gson `@SerializedName`-annotated classes, Kotlin `data class` |
| Enums | Java/Kotlin `enum class Name { A, B, C }`, including constructor-arg members like `PENDING("P")` |

## Native iOS (Objective-C / Swift)

Detected via `*.xcodeproj`/`project.pbxproj`/`Podfile`, or the presence of `.swift`/`.m` source files.

| Aspect | Where to Look |
|--------|--------------|
| Screens | Swift `class X: UIViewController` (also `UITableViewController`/`UICollectionViewController`); Objective-C `@interface X : UIViewController`; storyboard scenes via `customClass="X"` in `*.storyboard`/`*.xib` |
| Fields | `@IBOutlet` declarations — Objective-C `@property (weak, nonatomic) IBOutlet UIButton *saveButton;`, Swift `@IBOutlet weak var saveButton: UIButton!` |
| API calls | `URLSession`/`Alamofire`/`AFNetworking` calls; URL string literals (`"https://..."` in Swift, `@"https://..."` in Objective-C) with the HTTP method inferred from nearby context |
| Models | Swift `struct`/`class` (especially `Codable`/`Decodable`/`Encodable` conformance, or names ending `Model`/`VO`/`DTO`/`Response`/`Request`/`Entity`); Objective-C `@interface X : NSObject` with `@property` field declarations |
| Enums | Swift `enum Name { case a, case b(String) }`; Objective-C `typedef NS_ENUM(NSInteger, Name) { ... }` |
