...
def _humanize_word(word: str) -> str:
    ...
    return " ".join(w[:1].upper() + w[1:] for w in word.split() if w)


def route_to_page_name(route: str) -> str:
    """Convert a route path — or a mobile/native 'screen' identifier (Activity/
    ViewController class name, WCF operation name, ASP.NET .aspx filename) —
    into a human-readable page name."""
    if route in ("/", ""):
        return "Home"
    # Strip web-page file extensions (ASP.NET Web Forms) before word-splitting
    route = re.sub(r"\.(aspx|ascx|cshtml)$", "", route, flags=re.IGNORECASE)
    parts = route.strip("/").split("/")
    # Remove dynamic segments for naming: :param, *param, {param} (.NET route templates)
    clean = [p for p in parts if not p.startswith(":") and not p.startswith("*")
             and not re.fullmatch(r"\{[^}]*\}", p)]
    if not clean:
        clean = [re.sub(r"^[:*]|^\{|\}$", "", p) for p in parts]
    words = "-".join(clean).replace("_", "-").split("-")
    return " ".join(_humanize_word(w) for w in words if w) or "Home"
