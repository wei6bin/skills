...
- **Agents** (`agents/`) — subagents the orchestrator dispatches via the task tool with `agent_type: "prd-pr:<name>"` (or `prd-pr-copilot:<name>`): `code-explorer`, `code-architect`, `plan-reviewer`, `impl-backend`, `impl-frontend`, `impl-simplify`, `code-reviewer`, `security-reviewer`, `test-plan-walker`.
