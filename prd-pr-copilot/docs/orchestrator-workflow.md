...
flowchart TD
    ...
    class P2C,P4B,P5W,P9T,P10A skill
...
| 8 Slice-by-slice Impl | per slice: `impl-backend` → `impl-frontend` → `impl-simplify` (append the reuse mode from the slice's story-point size to each implementer scope — `reuse: lite` for 1–2 pts, `reuse: full` for 3+; large slices stay `full` and flag over-scope, never dropping an AC) | `vertical-slicing`, `backend-implementer`, `frontend-implementer`, `react-best-practices`, `restful-api-design`, `context-updater` (invoked by impl agents) | Code + tests + checkpoint commits |
