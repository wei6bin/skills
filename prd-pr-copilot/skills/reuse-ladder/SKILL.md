...
Never take a shortcut *through* the guardrails: input validation, error handling that prevents data loss (and error/empty/loading states in the UI), security, and accessibility (roles, labels, keyboard) are **never** simplified away, whatever the lean mode.
